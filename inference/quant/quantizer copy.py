...
def lowbit_gemm(x_int, w_int, a_scale, w_scale, bias=None, symmetric_a=True, symmetric_w=True):
    """
    Placeholder for REAL low-bit GEMM kernel.
    Current behavior: dequantize to float and call F.linear.
    Replace this body with your CUDA kernel later.
    """
    # dequantize activations
    if symmetric_a:
        x_fp = (x_int.float()) * a_scale
    else:
        raise NotImplementedError("asymmetric act dequant not implemented here")

    # dequantize weights
    if symmetric_w:
        w_fp = (w_int.float()) * w_scale
    else:
        raise NotImplementedError("asymmetric weight dequant not implemented here")

    return F.linear(x_fp, w_fp, bias)
...
class QuantLinear(nn.Module):
    ...
    def __init__(
        ...
        with torch.no_grad():   
            ...
            if self.mode == "pseudo":
                ...
                # for completeness, keep scales if you want to log/inspect
                self.register_buffer("w_scale", scale_w)
                if self.use_zero_point:
                    self.register_buffer("w_zp", zp_w)
                else:
                    self.w_zp = None
            else:  # "real"
    ...
    def forward(self, x):
        ...
        if self.mode == "pseudo":
            ...
            # If a_bit is not set (<16 is our quant region), we fallback to dequant-weight + F.linear
            if self.a_bit is None or self.a_bit >= 16:
                # light fallback to pseudo on activation
                w_fp = (self.w_int.float()) * self.w_scale
                return F.linear(x, w_fp, self.bias)

            # quantize activation to int (symmetric only here)
            if self.use_zero_point:
                raise NotImplementedError("real mode currently supports symmetric only")
            x_int, a_scale, _ = _quantize_to_int(x, self.a_bit, group_size=self.group_size, symmetric=True)

            # call lowbit GEMM placeholder (replace later with CUDA kernel)
            return lowbit_gemm(
                x_int=x_int,
                w_int=self.w_int,
                a_scale=a_scale,
